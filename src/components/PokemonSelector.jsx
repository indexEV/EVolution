...
function useMarqueeOnOverflow(deps = []) {
  ...
  useLayoutEffect(() => {
    ...
    const scheduleCheck = () => {
      ...
      rafIdRef.current = requestAnimationFrame(checkOverflow);
    };

    const rafIdRef = { current: 0 };
    // Step 3's header width settles slightly later because of the shiny-button layout.
    // Observe the wrapper so marquee re-checks after that final resize instead of only on initial render.
...
function getModifierPills(selectedAbility, status, entryPillDefs, effectiveAbility, pokemonId) {
  ...
  const hasEntryLabel = label => (entryPillDefs ?? []).some(p => p.label === label);
  ...
  if (ab === 'intimidate' && !hasEntryLabel('Intimidate'))
                                push('intimidate',      'Intimidate',     { role: 'attacker' });
...
const PokemonSelector = forwardRef(({ title, onSelect, selectedPokemon, collapsed, step = 1, onAbilityError, opponentInfo, opponentFullState, opponentPokemon, onStateChange, level = 100, onLevelChange = null, fieldConditions = null }, ref) => {
  ...
  useEffect(() => {
    ...
    // Copied Intimidate should behave like the copied pokemon's modifier state.
    if (ab === 'intimidate') toEnable.add(ab);
  ...
  useEffect(() => {
    if (_normAb(tracedAbility) === 'intimidate') return;
    setActiveModifiers(prev => {
      if (!prev.has('intimidate')) return prev;
  ...
  useEffect(() => {
    ...
    const lockedName = selectedPokemon.requiredItem ?? selectedPokemon.requiredItems?.[0] ?? null;
    if (lockedName) {
  ...
  useImperativeHandle(ref, () => ({
    ...
    getFullState: () => {
      ...
      const calcBaseStages = copyStats
        ...
        : statStages;
      const effectiveStages = computeEffectiveStages(
        calcBaseStages,
        opponentInfo,
  ...
  }, [selectedPokemon, userEvs, userIvs, statStages, collapsed, selectedNature, level, opponentInfo?.intimidateActive, activeModifiers, stackCounts, opponentPokemon, opponentFullState, selectedAbility, tracedAbility, poaPickedAbility]);
  ...
  useEffect(() => {
    if (calculatedStats && selectedPokemon) {
      ...
      const intimidateActive = intimidatePill ? activeModifiers.has(intimidatePill.id) : copiedIntimidateActive;
      ...
      {selectedPokemon ? (
        ...
        >
          ...
          <div className={`pokemon-header ${collapsed ? 'collapsed' : ''} ${step === 3 ? 'has-shiny-btn' : ''}`}>
            ...
            {/* Moves + Spread column — keep visible through Step 7 for context */}
            {step >= 5 && (
              <div className="collapsed-right-section">
                ...
                <div className="collapsed-moves">
                  {[0,1,2,3].map(i => {
                    ...
                    const TYPE_COLORS = {
                      normal:'#D4D1AE', fire:'#FFB067', water:'#9FC4FF', electric:'#FFE27A',
                      grass:'#9CE57D', ice:'#C6F1F1', fighting:'#F06C67', poison:'#D98AE1',
                      ground:'#F2DB93', flying:'#C1A9FF', psychic:'#FF8DB7', bug:'#C9DC63',
                      rock:'#D3B95E', ghost:'#B49BDE', dragon:'#A58CFF', dark:'#B69B8E',
                      steel:'#D8D8F0', fairy:'#FFC0D4', stellar:'#8DEAEA',
                    };
            ...
            {selectedPokemon.baseStats && (
              <>
                {statNames.map((stat, idx) => {
                  ...
                  const effectiveAbilityForRow = tracedAbility ?? poaPickedAbility ?? selectedAbility;
                  ...
                  const effStages = collapsed
                    ? computeEffectiveStages(displayStages, opponentInfo, entryPillDefsRef.current, activeModifiers, stackCounts, displayPoke?.baseStats, effectiveAbilityForRow)
                    : statStages;
                  ...
                  <div key={stat} className={`stat-row ${collapsed ? 'input-mode' : ''}`}>
                    ...
                    <span className="stat-value">{stat === 'hp' ? selectedPokemon.baseStats[stat] : (displayPoke?.baseStats?.[stat] ?? selectedPokemon.baseStats[stat])}</span>
